...
def betas_for_alpha_bar(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
    ...
    for i in range(num_diffusion_timesteps):
        ...
        # 同时还要控制上界，因此有个min，最大的beta是0.999，alpha_bar对应iddpm的f函数
...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if isinstance(model_output, tuple):
            # 检查这个模型的输出是否是一个元祖类型，这样可以把原本的输出拆成主输出和辅助输出
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            # 如果方差是可学习的，那么输出就是2倍的channel，包含mean 和 Var，接下来做拆分即可
            ...
            # 不可学习的方差
            # 这里也有两种选择，你可以使用 \beta_{t} 也可以用 \{\bar beta}_{t}
        ...
        if self.model_mean_type == ModelMeanType.START_X:
            ...
            # 预测的是eps，那么调用函数拿到x0，公式13
            pred_xstart = process_xstart(
                self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
            )
        # 有了x_t, x_0，那么继续算出对应的 x_{t-1}
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample
            samples, _ = final["sample"].chunk(2, dim=0)
            sample_buffer.append(samples)
            # print('sample_buffer:', len(sample_buffer))
            # print('sample_buffer[-1].shape:', sample_buffer[-1].shape)
        return final["sample"], sample_buffer
    ...
    def ddim_sample(
        ...
        if model_kwargs['gen_type'] == 'ddd':
            if t[0] >= model_kwargs['low'] and t[0] <= model_kwargs['high']:
                instep = True
            else:
                instep = False

        if model_kwargs['gen_type'] == 'ddd' and instep:
            # print('t[0]:',t[0])
            predict_start, _ = out["pred_xstart"]
            guidance_score = - (predict_start - model_kwargs['syn_feature']) * model_kwargs['feat_scale'] * th.exp(0.5 * out["log_variance"])
            # guidance_score = - (predict_start - model_kwargs['syn_feature']) * model_kwargs['feat_scale']
            sample = sample + guidance_score
        return {"sample": sample, "pred_xstart": out["pred_xstart"]}
